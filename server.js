...
app.post('/api/kirim', (req, res) => {
  upload.array('photos', 3)(req, res, async (uploadErr) => {
    if (uploadErr) {
...
// ── 404 & error handler — selalu return JSON, bukan HTML ──
app.use((req, res) => {
  res.status(404).json({ ok: false, message: `Route tidak ditemukan: ${req.method} ${req.path}` });
});
